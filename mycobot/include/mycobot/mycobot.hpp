...
class MyCobot {
 ...
 public:
  ...
  MyCobot(std::string const& port = "/dev/ttyAMA0", uint32_t baudrate=115200)
    : serial_port_{std::make_unique<serial::Serial>(port, baudrate,
   ...
   */
  MyCobot(std::unique_ptr<serial::Serial> serial_port) : serial_port_{std::move(serial_port)} {}
   ...
   *
   * @return     Error if encountered, result if there is one, otherwise empty repose
   */
